...
def _patch_dispatch_basics(mock_registry, *, mcp_return=("raw output", 100), mcp_side_effect=None):
    ...
    return (
        ...
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.single_mcp_call", **mcp_kwargs),
        patch(
...
async def test_dispatch_passes_identical_task_goal_to_both(server_a, server_b):
    ...
    with (
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.registry", mock_registry),
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.single_mcp_call", mock_mcp_call),
        patch(
            "backend.tool_arena.comparison.ask_two_tools_concurrently.sanitize_output",
            side_effect=lambda text, servers: text,
        ),
...
async def test_sanitize_called_on_each_result(server_a, server_b):
    ...
    with (
        ...
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.sanitize_output", side_effect=track_sanitize),
    ):
...
async def test_timeout_failure_isolation(server_a, server_b):
    ...
    with (
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.registry", mock_registry),
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.single_mcp_call", side_effect=mcp_call_side_effect),
        patch(
            "backend.tool_arena.comparison.ask_two_tools_concurrently.sanitize_output",
            side_effect=lambda text, servers: text,
        ),
...
async def test_dispatch_reads_timeout_from_env(server_a, server_b, monkeypatch):
    ...
    from backend.tool_arena.rag_tool.list_available_tools import registry as _real_registry
    ...
    with (
        ...
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.asyncio.wait_for", side_effect=capture_wait_for),
    ):
        import importlib

        import backend.tool_arena.comparison.ask_two_tools_concurrently as dispatcher_module
...
async def test_dispatch_retries_on_transient_connection_error(server_a, server_b):
    ...
    with (
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.registry", mock_registry),
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.single_mcp_call", side_effect=flaky_call),
        patch(
            "backend.tool_arena.comparison.ask_two_tools_concurrently.sanitize_output",
            side_effect=lambda text, servers: text,
        ),
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.asyncio.sleep", new_callable=AsyncMock),
    ):
        result_a, result_b = await MCPDispatcher().dispatch(
            task="t", goal="g", session_id="s",
        )
...
async def test_dispatch_does_not_retry_on_timeout(server_a, server_b):
    ...
    with (
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.registry", mock_registry),
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.single_mcp_call", side_effect=slow_call),
        patch(
            "backend.tool_arena.comparison.ask_two_tools_concurrently.sanitize_output",
            side_effect=lambda text, servers: text,
        ),
...
async def test_dispatch_raises_when_all_servers_needs_reauth(server_a, server_b):
    ...
    from backend.tool_arena.rag_tool.readiness import ServerStatus
...
async def test_dispatch_excludes_unready_from_pool(server_a, server_b):
    ...
    with (
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.registry", mock_registry),
        patch("backend.tool_arena.comparison.ask_two_tools_concurrently.single_mcp_call", side_effect=track),
        patch(
            "backend.tool_arena.comparison.ask_two_tools_concurrently.sanitize_output",
            side_effect=lambda text, servers: text,
        ),
